...
async function init() {
    ...
    return new Promise((acc, rej) => {
        pool.query(
            'CREATE TABLE IF NOT EXISTS todo_items (id varchar(36), name varchar(255), completed boolean, priority integer, due_date DATETIME, category varchar(255)) DEFAULT CHARSET utf8mb4',
            err => {
...
async function getItems() {
    return new Promise((acc, rej) => {
        pool.query('SELECT * FROM todo_items ORDER BY id DESC', (err, rows) => {
            if (err) return rej(err);
            acc(
                rows.map(item =>
                    Object.assign({}, item, {
